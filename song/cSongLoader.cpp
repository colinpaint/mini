...
class cDvbService {
public:
  cDvbService (int sid, bool selected) : mSelected(selected) { (void)sid; }  // mSid(sid),
...
public:
  ...
  // ignore any leading payload before a payloadStart
    ...
    if (hasPayload && (payloadStart || mGotPayloadStart)) {
      ...
      payload (ts + headerSize, 188 - headerSize, payloadStart, continuityCount, reuseFromFront);
      }
    }
  //}}}
...
protected:
  //{{{
  virtual void payload (uint8_t* ts, int tsLeft, bool payloadStart, int continuityCount, bool reuseFromFront) {
...
public:
  ...
  cPmtParser (int pid, int sid, function<void (int streamSid, int streamPid, int streamType)> callback)
    : cPidParser (pid, "pmt"), mCallback(callback) { (void)sid; } // mSid(sid),
  //}}}
...
public:
  ...
  cPesParser (int pid, const string& name, bool useQueue) : cPidParser(pid, name), mUseQueue(useQueue) {
    ...
    if (useQueue)
      thread ([=,this](){ dequeThread(); }).detach(); // ,this
    }
  //}}}
  //{{{
  virtual ~cPesParser() {
...
protected:
  ...
  // ts[8] = optional header length
    ...
    if ((mContinuityCount >= 0) &&
        (continuityCount != ((mContinuityCount + 1) & 0xF))) // !!! should abandon pes here !!!!
...
public:
  ...
  virtual void load (function <void(int64_t)>& playCallback) = 0;
...
public:
  ...
  virtual void load (function <void(int64_t)>& playCallback) final { (void)playCallback; }
...
public:
  ...
  //}}}
  //{{{
  virtual void load (function <void(int64_t)>& playCallback) final {
    ...
    mPtsSong->setPlayCallback (playCallback);
    ...
    auto audioFrameCallback = [&](bool reuseFromFront, float* samples, int64_t pts) noexcept {
      ...
      if (waitForPts) {
        ...
        waitForPts = false;
        cLog::log (LOGINFO, fmt::format ("resync pts:{}", utils::getPtsFramesString (pts, mPtsSong->getFramePtsDuration())));
        }
      loadPts = pts;
      };
    //}}}
            ...
            while (cAudioParser::parseFrame (buffer, bufferEnd, frameSize)) {
              ...
              if (samples) {
                if (!mSong) {
                  ...
                  mSong->setPlayCallback (playCallback);
                  }
...
public:
  ...
  virtual void load (function <void(int64_t)>& playCallback) final {
    ...
    mHlsSong->setPlayCallback (playCallback);
    ...
    while (!mExit) {
      ...
      // get m3u8 file
      string m3u8Path = fmt::vformat (mM3u8PathFormat, fmt::make_format_args(mChannel, mAudioRate, mVideoRate));
      ...
      if (http.getContent()) {
        ...
        while (!mExit) {
          ...
          if (chunkNum > 0) {
            ...
            string tsPath = fmt::vformat (mTsPathFormat, fmt::make_format_args(mChannel, mAudioRate, mVideoRate, chunkNum));
...
// cSongLoader load
//{{{
void cSongLoader::load (const vector<string>& params, function <void(int64_t)>& playCallback) {
// run recognised load
...
//}}}
//{{{
void cSongLoader::launchLoad (const vector<string>& params, function <void(int64_t)>& playCallback) {;
